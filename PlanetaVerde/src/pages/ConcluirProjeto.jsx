...
function ConcluirProjeto() {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(`${API_BASE}/projetos/concluir`, {
        headers: {
  ...
  const enviarFormulario = async (e) => {
    ...
    const url = editando
      ? `${API_BASE}/projetos/concluir/${editando.id}`
      : `${API_BASE}/projetos/concluir`;
    ...
    try {
      ...
      toast.success(editando ? "Projeto atualizado!" : "Projeto concluído!");
      setFormData({
        nome_projeto: "",
        data_inicio: "",
        data_conclusao: "",
  ...
  const handleEdit = (projeto) => {
    ...
    setFormData({
      ...
      data_conclusao: projeto.data_conclusao
        ? projeto.data_conclusao.slice(0, 16)
        : "",
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`${API_BASE}/projetos/concluir/${id}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },
    ...
    <div className="page-container">
      ...
      <div className="container">
        ...
        {projetos.length > 0 ? (
            ...
            <tbody>
              {projetos.map((proj) => (
                <tr key={proj.id}>
                  ...
                  <td>
                    {proj.data_conclusao
                      ? new Date(proj.data_conclusao).toLocaleString("pt-BR")
                      : "-"}
                  </td>
          ...
          <p>Nenhum projeto encontrado para concluir.</p>
        )}
      </div>
      <Footer />
    </div>
...
export default ConcluirProjeto;